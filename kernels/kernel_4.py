...
def attn_kernel(
    ...
    # program ids
    # -----------------------------
    pid_kv = tl.program_id(0)   # kv split
    pid_x  = tl.program_id(1)   # Q block
    pid_d  = tl.program_id(2)   # D split

    # -----------------------------
    # derive split range
    # -----------------------------
    kv_tiles_per_split = (T_c + KV_SPLIT - 1) // KV_SPLIT

    start_tile = pid_kv * kv_tiles_per_split
    end_tile   = tl.minimum(start_tile + kv_tiles_per_split, T_c)

    # -----------------------------
    # batch/head (only 1 kv_head here)
    # -----------------------------
    batch = 0
    kv_head = 0
...
def main():
    ...
    grid1 = (KV_SPLIT, triton.cdiv(S, B_r), N_d)
